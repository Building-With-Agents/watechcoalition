...
## What I Found
* Using only the pure hash (source + ID) found 0 duplicates. This confirms that JSearch and scraped career pages use different IDs for the same job, which makes them invisible to exact matching logic.
* The hybrid strategy successfully found the 5 injected duplicates. By using a logical fingerprint (title, company, date), we bridged the gap between different platforms.
...
* **Collision Risk:** There is a chance two different jobs at the same company on the same day could match, but catching cross-source duplicates is a higher priority for data quality.
...
* **`test_generate_logical_hash_normalization`**: Confirmed a **0% False Positive rate**; normalization ensures "Engineer" and "engineer" are treated as identical while ignoring source IDs.
