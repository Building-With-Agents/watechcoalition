...
- **Added** `agents/pipeline_runner.py`: run the stub chain end-to-end (trigger → Ingestion → … → Orchestration; Demand Analysis returns None and is skipped). Run from repo root: `python -m agents.pipeline_runner` (requires `cd agents && pip install -r requirements.txt`).
- **Added** `agents/tests/test_pipeline_runner.py`: pytest for 2.2 stubs (correlation_id preservation, Demand Analysis returns None, full chain, health_check shape). Run: `cd agents && pytest tests/test_pipeline_runner.py -v`.

### Task 2.3 — Sequential pipeline runner
