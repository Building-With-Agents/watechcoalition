...
class EmergenceAlertEvent(BaseModel):
    """Typed wrapper for EmergenceAlert control events (orchestration-only subscriber)."""
    ...
    @classmethod
    def check_event_type(cls, v: EventEnvelope) -> EventEnvelope:
        if v.payload.get("event_type") != "EmergenceAlert":
            raise ValueError(f"Expected payload event_type 'EmergenceAlert', got {v.payload.get('event_type')!r}")
        return v
