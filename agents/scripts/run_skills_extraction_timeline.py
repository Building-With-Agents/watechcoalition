...
def _payload_summary(payload: dict, max_skills: int = 5, max_tools: int = 5) -> dict:
    ...
    if skills:
        out["skills_preview"] = [s.get("skill_name") or s.get("label") or s.get("name") for s in skills[:max_skills]]
    ...
    for s in skills_full:
        ...
        parts.append(
            '<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td class="span-cell">{}</td></tr>'.format(
                html.escape(str(s.get("skill_name") or s.get("label") or s.get("name") or "")),
