# Extraction eval run — 2026-03-23T23:14:34-06:00 (America/Denver)

## Metadata

- **Run label:** v2-softskill-filter
- **Git (short):** ca8d7629
...
- **Snapshot schema:** `1.0`
- **Wall runtime (s):** 667.857

## Aggregate metrics

- **Skills** — precision: 0.1327, recall: 0.1855
- **Skills counts** — GT: 221, pred: 309, matched: 41
- **Tools** — precision: 0.7603, recall: 0.7244
- **Tools counts** — GT: 127, pred: 121, matched: 92
- **Total tokens (skills LLM):** 49366
- **Total cost USD (estimate):** 0.26292
- **Total latency ms (sum per job):** 555561
...
- Prefer the most specific actionable skill grounded in the text. Do not emit a generic umbrella skill when a more specific domain or task skill is supported.
- Suppress generic soft skills and hiring boilerplate unless the text gives domain-specific context. Bare mentions of communication, leadership, collaboration, teamwork, problem solving, detail orientation, adaptability, multitasking, fast learner, self-starter, positive attitude, or similar traits should usually be skipped.
- If a generic soft-skill word appears with concrete domain context, extract the specific contextualized skill label instead of the generic word. Example: extract "Stakeholder Communication", not "Communication".
...
- If unsure whether a soft skill is specific enough, skip it.
...
- "Stakeholder communication for cross-functional alignment" -> EXTRACT "Stakeholder Communication" (Soft)
- "Lead sprint planning, daily standups, and unblock engineers" -> EXTRACT "Scrum Facilitation" or "Agile Facilitation"; do NOT extract generic "Leadership"
- "Investigate production incidents and perform root-cause analysis" -> EXTRACT "Root Cause Analysis"; do NOT extract generic "Problem Solving"
